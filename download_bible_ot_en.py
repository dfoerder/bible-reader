#!/usr/bin/env python3
"""
Downloads the Old Testament (World English Bible) from the getBible API
and saves it as bible_ot_en.json
"""
import json
...
def download_book(book_nr):
    ...
    try:
        ...
        print(f"  Warning: Error downloading book {book_nr}: {e}")
        return None

def main():
    print("=" * 50)
    print("  Old Testament download")
    ...
    for i, book_nr in enumerate(OT_BOOKS, 1):
        ...
        print(f"  [{i}/{total}] Done: {book_name} -- {chap_count} chapters, {verse_count} verses")
        time.sleep(0.3)
    ...
    with open("bible_ot_en.json", "w", encoding="utf-8") as f:
    ...
    print(f"  Saved: bible_ot_en.json ({file_size:.1f} MB)")
    print()
